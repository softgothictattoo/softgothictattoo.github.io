...
- The header bar is `img/header-collage.jpg`, 24 pieces from
  `img/tattoos/` in a 12x2 grid, desaturated and darkened. It is sized to
  the bar's height and repeated across, not `cover`, so whole miniatures
  show rather than a cropped slice of limbs. White nav text sits on it, so
  keep the darkening and the `::before` overlay together: they put the
  worst case at 6:1. Rebuild it by re-running the montage over a new
  selection, then re-check contrast before pushing.
